    ...
    pub(super) fn step(&mut self, mode: InsertionMode, token: Token) -> ProcessResult<Handle> {
        ...
        match mode {
            ...
            BeforeHtml => match token {
                ...
                CharacterTokens(Whitespace, _) => Done,
                CommentToken(text) => self.append_comment_to_doc(text),
                crate::tree_builder::types::TagToken(
                    ...
                    match (enable_wildcards, last_arm_token) {
                        (_, token) => {
                            ...
                            let contains_nonspace =
                                pending.iter().any(|&(split, ref text)| match split {
                                    Whitespace => false,
                                    NotWhitespace => true,
