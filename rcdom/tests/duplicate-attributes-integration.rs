...
/// A TreeSink that captures had_duplicate_attributes flags for created elements
pub struct FlagCapturingDOM {
    pub had_duplicate_attributes_flags: RefCell<Vec<bool>>,
    pub rcdom: RcDom,
}
...
impl FlagCapturingDOM {
    pub fn new() -> Self {
        FlagCapturingDOM {
            had_duplicate_attributes_flags: RefCell::new(Vec::new()),
            rcdom: RcDom::default(),
        }
    }
}
...
impl TreeSink for FlagCapturingDOM {
    ...
    fn create_element(&self, name: QualName, attrs: Vec<Attribute>, flags: ElementFlags) -> Handle {
        // Capture the had_duplicate_attributes flag value for inspection
        self.had_duplicate_attributes_flags
            .borrow_mut()
            .push(flags.had_duplicate_attributes);
...
fn test_duplicate_attrs_flag_set() {
    ...
    // Find if any element has had_duplicate_attributes=true
    let flags = sink.had_duplicate_attributes_flags.borrow();
    let has_duplicate = flags.iter().any(|&f| f);

    assert!(
        has_duplicate,
        "Expected to find an element with had_duplicate_attributes=true"
    );
}
...
fn test_no_duplicate_attrs_flag_not_set() {
    ...
    // All flags should have had_duplicate_attributes=false
    let flags = sink.had_duplicate_attributes_flags.borrow();
    for flag in flags.iter() {
        assert!(
            !flag,
            "Expected had_duplicate_attributes to be false for elements without duplicates"
        );
    }
}
...
fn test_multiple_duplicates_sets_flag() {
    ...
    assert!(
        has_duplicate,
        "Expected to find an element with had_duplicate_attributes=true for multiple duplicates"
    );
}
...
fn test_script_with_duplicate_nonce() {
    ...
    assert!(
        has_duplicate,
        "Expected script with duplicate nonce to have had_duplicate_attributes=true"
    );
}
